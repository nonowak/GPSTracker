...
export class MapService {
  ...
  getMeasurements(deviceId) {
    ...
    this._http.get('http://a75192fc.eu.ngrok.io/gps/measurements/' + deviceId.toString(), {headers: headers})
      .toPromise()
      .then(response => {
  ...
  getFirstAndLastMeasurement(deviceId): Observable<FirstAndLastMeasurementDateDTO> {
    const headers = new Headers({'Authorization': 'Bearer ' + Cookie.get('access_token')});
    return this._http.get('http://a75192fc.eu.ngrok.io/gps/measurements/'
